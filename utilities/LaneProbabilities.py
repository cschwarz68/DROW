...
for filename in filenames:
    ...
    for col in df:
        if col == ('vtti.left_marker_probability'):
            ...
            # % of the time machine vision is full
            percentage_perfect_left = float(len(unique_values_left)) / float(len(column))*100
            left_cam_rows = np.array([mean_left, standard_deviation_left,
                                      percentage_perfect_left])
        ...
        if col == ('vtti.right_marker_probability'):
            ...
            right_cam_rows = np.array([mean_right, standard_deviation_right,
                                       percentage_perfect_right])

    # add ambient light level metrics to the dataframe
    columns = ['left_marker_mean', 'left_marker_stdv',
               'left_marker_%perfect', 'right_marker_mean',
               'right_marker_stdv', 'right_marker_%perfect',
               'light_min', 'light_max']
    for col in df:
