...
def main():
    for filename in filenames:
        ...
        df = pd.read_csv(filename,
                         usecols=['vtti.file_id', 'vtti.speed_network',
                                  'vtti.accel_x', 'vtti.accel_y',
                                  'vtti.gyro_z',
                                  'vtti.lane_distance_off_center',
                                  'vtti.left_marker_probability',
                                  'vtti.right_marker_probability',
                                  'TRACK1_TARGET_ID', 'TRACK2_TARGET_ID',
                                  'TRACK3_TARGET_ID', 'TRACK4_TARGET_ID',
                                  'TRACK5_TARGET_ID', 'TRACK6_TARGET_ID',
                                  'TRACK7_TARGET_ID', 'TRACK8_TARGET_ID',
        ...
        for idx in range(len(df)):
            ...
            for number in ypos_row:
                if abs(number) == min(abs(x) for x in ypos_row if x
                                      is not None):
                    min_ypos = number
                    break
                else:
